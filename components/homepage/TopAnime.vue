<template>
  <aside class="w-full flex flex-col">
    ...
    <template v-else>
      <section id="aside-header" class="w-full flex items-center justify-center py-1.5 bg-mal-lightdark border-b border-mal-dark rounded-t-sm">
      ...
      <section id="aside-content" class="w-full" v-if="animeData && animeData.length > 0">
        <swiper-container
          ...
          free-mode="true"
        >
          <swiper-slide lazy="true" class="w-[120px]" v-for="(anime, index) in animeData" :key="anime.mal_id">
            <nuxt-link to="#">
              <div class="relative">
                <img :src="anime.images.webp.image_url" alt="anime poster" class="w-[120px] h-[180px] object-cover z-0 border border-mal-lightdark">
                <div class="absolute z-10 bottom-0 inset-x-0 px-2 pb-2 bg-gradient-to-b from-transparent to-black/90">
                ...
                <p class="absolute top-2 left-2 w-max px-1.5 py-0.5 bg-mal-blue text-white text-xs border border-white rounded-sm">
                  {{ index + 1 }}
                </p>
              </div>
            </nuxt-link>
